...
rule pupy_primers_align:
    input:
        target_cds = "data/pupy_primers/{genus}/target",
    ...
    log: "logs/pupy_primers_align/{genus}.log"
    benchmark: "benchmarks/pupy_primers_align/{genus}.txt"
    resources: cpus = 24, time_min=20000, mem_mb = 100000
    shell:
        """
        rm -r $(dirname {output.align_out}) # pupy doens't like folder to exist

        puppy-align \
            -pr {input.target_cds} \
            -nt {input.non_target_cds} \
            -o $(dirname {output.align_out}) 2>&1 | tee {log}
        """
...
rule pupy_primers:
    ...
    shell:
        ...
        puppy-primers \
            ...
            -pr {input.target_cds} \
            -i {input.align_out} \
            -o {output.primers_out} 2>&1 | tee {log}
        """