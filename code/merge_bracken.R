...
#                                   "--counts-out=data/sample_data/bracken_counts.tsv",
#                                   "--rel-out=data/sample_data/bracken_rel_abund.tsv"))
# print(arguments)
...
bracken_files <- system("ls $PWD/data/omics/metagenomes/*/kraken_fastp/*_bracken.txt", intern = TRUE) %>% 
...
wide_counts <- abund_mat %>% 
  select(taxonomy_id, taxonomy, database, all_of(tax_levels),new_est_reads) %>% 
...
wide_rel_abund <- abund_mat %>% 
  select(taxonomy_id, taxonomy, database, all_of(tax_levels),rel_abund) %>% 
