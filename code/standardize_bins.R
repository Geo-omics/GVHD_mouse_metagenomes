...
Usage:
  standardize_bins.R --sample=SAMPLE
...
arguments <- docopt(doc, args = c("--sample=coassembly"))
#print(arguments)
...
metabat_bins <- system(paste0("ls data/omics/metagenomes/",arguments$sample,"/bins/METABAT2/*.fa"), intern = TRUE) %>% 
  data.frame(orig_bin_path = .) %>% 
  mutate(sample = orig_bin_path %>% str_extract("(?<=data/omics/metagenomes/).*(?=/bins/METABAT2/.*)"),
         orig_bin_name_w_ext = orig_bin_path %>% str_remove(".*METABAT2/"),
         orig_bin_name = orig_bin_name_w_ext %>% str_remove("\\.[a-z]*$"),
         binner = "metabat2")

concoct_bins <- system(paste0("ls data/omics/metagenomes/",arguments$sample,"/bins/CONCOCT/output/fasta_bins/*.fa"), intern = TRUE) %>% 
  data.frame(orig_bin_path = .) %>% 
  mutate(sample = orig_bin_path %>% str_extract("(?<=data/omics/metagenomes/).*(?=/bins/CONCOCT/output/fasta_bins/.*)"),
         orig_bin_name_w_ext = orig_bin_path %>% str_remove(".*fasta_bins/"),
         orig_bin_name = orig_bin_name_w_ext %>% str_remove("\\.[a-z]*$"), 
         binner = "concoct")

maxbin_bins <- system(paste0("ls data/omics/metagenomes/",arguments$sample,"/bins/maxbin/*.fasta"), intern = TRUE) %>% 
  data.frame(orig_bin_path = .) %>% 
  mutate(sample = orig_bin_path %>% str_extract("(?<=data/omics/metagenomes/).*(?=/bins/maxbin/.*)"),
         orig_bin_name_w_ext = orig_bin_path %>% str_remove(".*maxbin/"),
         orig_bin_name = orig_bin_name_w_ext %>% str_remove("\\.[a-z]*$"), 
         binner = "maxbin")

semibin_bins <- system(paste0("ls data/omics/metagenomes/",arguments$sample,"/bins/semibin/output_bins/*.fa"), intern = TRUE) %>% 
  data.frame(orig_bin_path = .) %>% 
  mutate(sample = orig_bin_path %>% str_extract("(?<=data/omics/metagenomes/).*(?=/bins/semibin/output_bins/.*)"),
         orig_bin_name_w_ext = orig_bin_path %>% str_remove(".*output_bins/"),
         orig_bin_name = orig_bin_name_w_ext %>% str_remove("\\.[a-z]*$"), 
         binner = "semibin")

VAMB_bins <- system(paste0("ls data/omics/metagenomes/",arguments$sample,"/bins/VAMB/bins/*.fna"), intern = TRUE) %>% 
  data.frame(orig_bin_path = .) %>% 
  mutate(sample = orig_bin_path %>% str_extract("(?<=data/omics/metagenomes/).*(?=/bins/VAMB/.*)"),
         orig_bin_name_w_ext = orig_bin_path %>% str_remove(".*VAMB/"),
         orig_bin_name = orig_bin_name_w_ext %>% str_remove("\\.[a-z]*$"), 
         binner = "VAMB")

metadecoder_bins <- system(paste0("ls data/omics/metagenomes/",arguments$sample,"/bins/metadecoder/bins/*.fasta"), intern = TRUE) %>% 
  data.frame(orig_bin_path = .) %>% 
  mutate(sample = orig_bin_path %>% str_extract("(?<=data/omics/metagenomes/).*(?=/bins/metadecoder/.*)"),
...
all_bins <- bind_rows(metabat_bins, concoct_bins, maxbin_bins, semibin_bins, VAMB_bins, metadecoder_bins) %>% 
  ...
  mutate(bin_num = row_number(),
         ...
         per_sample_combined_path = glue::glue("data/omics/metagenomes/{sample}/bins/all_raw_bins/{new_bin_name}.fa"),
         all_combined_bin_path = glue::glue("data/omics/metagenomes/metagenome_bins/raw_combined_bins/{new_bin_name}.fa")) %>% 
  write_tsv(paste0("results/bins/",arguments$sample,"_bins.tsv"))
...
contig_info_paths <- system(paste0("ls data/omics/metagenomes/",arguments$sample,"/assembly/megahit/contigs_info.tsv"),intern = TRUE)
...
bin_contigs %>% write_rds(paste0("data/omics/metagenomes/",arguments$sample,"/bins/contig_bins.rds"))
...
system(paste0("rm data/omics/metagenomes/",arguments$sample,"/bins/all_raw_bins/*.fa"))
