...
ggtree::ggtree(sample_tree,layout='circular') %<+% (metadata %>% relocate(import_id)) +
  geom_tiplab(aes(label = str_glue("{gut_section} {if_else(replicate != 'other', replicate, '')}")),offset = 7) + 
  expand_limits(x = 75) +
  geom_tippoint(aes(color = plot_categories),size = 4) +
  scale_color_manual(values = colors) + 
  labs(color = "Treatment group") +
  theme(plot.margin = margin(100,100,100,100))

ggsave("results/community_distance_tree_circular.png", width = 4, height = 3, scale = 3)
ggsave("results/community_distance_tree_circular.pdf", width = 4, height = 3, scale = 3)
...
# Abundance heatmap
```{r}
genus_percent_abund <- rel_abund_long %>% 
  select(-Species) %>% 
  filter(!is.na(Phylum),
         !is.na(Genus),
          Genus != "") %>% 
  group_by(Domain,Phylum, Class, Order,Family, Genus, sample) %>% 
  summarise(percent_abund = sum(rel_abund)) %>% 
  left_join(read_metaG_metadata())

clr_genus_abund <- genus_abund %>% 
  ungroup() %>% 
  select(sample, percent_abund, Genus) %>% 
  distinct() %>% 
  pivot_wider(names_from = sample,values_from = percent_abund,values_fill = 0) %>%
  mutate(across(where(is.numeric), ~ compositions::clr(as.numeric(.x)))) %>% 
  pivot_longer(-Genus, names_to = "sample", values_to = "clr")

genus_abund <- genus_percent_abund %>% 
  left_join(clr_genus_abund) %>% 
  mutate(transplant_type = factor(transplant_type, levels = c("Syngeneic", "Allogeneic", "Naive"), ordered = TRUE))

genus_variance <- genus_abund %>% 
  group_by(Domain,Phylum, Class, Order,Family, Genus) %>%
  summarise(variance = var(clr)) %>% 
  arrange(desc(variance))

most_variant_genera <- genus_variance %>% 
  ungroup() %>% 
  slice_max(n = 50,order_by = variance, with_ties = FALSE) %>% 
  arrange(desc(variance))

variant_genera_vec  <- most_variant_genera$Genus %>% unique()

most_variant_genera_hclust <- genus_abund %>% 
  filter(Genus %in% variant_genera_vec) %>%
  ungroup() %>% 
  select(sample, percent_abund, Genus) %>% 
  distinct() %>% 
  pivot_wider(names_from = sample,values_from = percent_abund,values_fill = 0) %>% 
...
hclust_order <- most_variant_genera_hclust$labels[most_variant_genera_hclust$order]

sample_hclust <- genus_abund %>% 
  filter(Genus %in% variant_genera_vec) %>%
  ungroup() %>% 
  select(sample, percent_abund, Genus) %>% 
  distinct() %>% 
...
sample_hclust_order <- sample_hclust$labels[sample_hclust$order]

(sample_section_bar <- genus_abund %>% 
  mutate(sample = factor(sample, levels = (sample_hclust_order), ordered = TRUE)) %>% 
  ggplot(aes(sample, 1, fill = gut_section)) +
  geom_tile() +
  ggh4x::scale_x_dendrogram(hclust = sample_hclust,position = "top") +
  scale_fill_viridis_d(option = "mako",direction = -1) +
    ...
    labs(fill = "Gut section")
)
  
(sample_day_bar <- genus_abund %>% 
  mutate(sample = factor(sample, levels = (sample_hclust_order), ordered = TRUE)) %>% 
  ggplot(aes(sample, 1, fill = days_after_transplant)) +
  geom_tile(color = "grey70") +
  scale_fill_viridis_d(option = "rocket",direction = -1,begin = 0.25,end = 0.9) +
  theme_minimal() +
    theme(axis.ticks = element_blank(),
          axis.text = element_blank(),
          panel.grid = element_blank(),
          axis.title = element_blank(),
          plot.margin = margin(0, 0, 0, 0, "pt")) +
    labs(fill = "Days after transplant")
)

(sample_type_bar <- genus_abund %>% 
  mutate(sample = factor(sample, levels = (sample_hclust_order), ordered = TRUE)) %>% 
  ggplot(aes(sample, 1, fill = transplant_type)) +
  geom_tile(color = "grey70") +
  scale_fill_manual(values = c("#2083e6", "#f09f26", "#9a5ebd")) +
    theme_minimal() +
    theme(axis.ticks = element_blank(),
          axis.text = element_blank(),
          panel.grid = element_blank(),
...
(genus_abund_plt <- genus_abund %>% 
  filter(Genus %in% most_variant_genera$Genus) %>% 
  mutate(Genus = factor(Genus, levels = hclust_order, ordered = TRUE),
         sample = factor(sample, levels = (sample_hclust_order), ordered = TRUE)) %>% 
  ggplot(aes(sample, Genus,fill = percent_abund)) +
  geom_tile(color = "grey70") +
  #ggh4x::facet_nested(~plot_categories + gut_section,scales = "free_x") +
  scale_fill_viridis_c(trans = "log2",na.value = "#ffffff",labels = scales::label_number()) +
  #scale_fill_viridis_c(na.value = "#ffffff") +
  ggh4x::scale_y_dendrogram(hclust = most_variant_genera_hclust) +
  ggh4x::scale_x_dendrogram(hclust = sample_hclust) +
  theme_bw() +
...
genus_abund %>% 
  ...
  group_by(Genus) %>% 
  mutate(Genus = factor(Genus, levels = hclust_order, ordered = TRUE),
         z_score = scale(percent_abund),
         days_after_transplant = factor(str_glue("Day: {days_after_transplant}"), levels = c("Day: 7", "Day: 21", "Day: other"), ordered = TRUE)) %>% 
  ggplot(aes(sample, Genus,fill = percent_abund)) +
  geom_tile(color = "grey70") +
  #ggh4x::facet_nested(~plot_categories + gut_section,scales = "free_x") +
  ggh4x::facet_nested(~ days_after_transplant + transplant_type + str_wrap_factor(gut_section,5) ,scales = "free_x") +
  #scale_fill_viridis_c(na.value = "#ffffff") +
  ggh4x::scale_y_dendrogram(hclust = most_variant_genera_hclust) +
  scale_fill_viridis_c(trans = "log2",na.value = "#ffffff",labels = scales::label_number(accuracy = 0.01)) +
  theme_bw() +
    theme(axis.text.x = element_blank(),
...
ggsave("results/genera_highest_variance_heatmap_facets.png",width = 6.5, height = 4, scale = 2.5)
ggsave("results/genera_highest_variance_heatmap_facets.pdf",width = 6.5, height = 4, scale = 2.5)
ggsave("results/genera_highest_variance_heatmap_facets.eps",width = 6.5, height = 4, scale = 2.5)
...
genus_abund %>% 
  ...
  geom_tile(color = "grey70") +
  #ggh4x::facet_nested(~plot_categories + gut_section,scales = "free_x") +
  ggh4x::facet_nested(~ gut_section + days_after_transplant + transplant_type ,scales = "free_x") +
  #scale_fill_viridis_c(na.value = "#ffffff") +
  ggh4x::scale_y_dendrogram(hclust = most_variant_genera_hclust) +
  scale_fill_viridis_c(trans = "log2",na.value = "#ffffff",labels = scales::label_number(accuracy = 0.01)) +
  theme_bw() +
...
ggsave("results/genera_highest_variance_heatmap_facets2.png",width = 6.5, height = 4, scale = 2.5)
ggsave("results/genera_highest_variance_heatmap_facets2.pdf",width = 6.5, height = 4, scale = 2.5)
ggsave("results/genera_highest_variance_heatmap_facets2.eps",width = 6.5, height = 4, scale = 2.5)
...
genus_abund %>% 
  ...
  geom_tile(color = "grey70") +
  #ggh4x::facet_nested(~plot_categories + gut_section,scales = "free_x") +
  ggh4x::facet_nested(~ transplant_type + days_after_transplant + gut_section ,scales = "free_x") +
  #scale_fill_viridis_c(na.value = "#ffffff") +
  ggh4x::scale_y_dendrogram(hclust = most_variant_genera_hclust) +
  scale_fill_viridis_c(trans = "log2",na.value = "#ffffff",labels = scales::label_number(accuracy = 0.01)) +
  theme_bw() +
...
ggsave("results/genera_highest_variance_heatmap_facet3.png",width = 6.5, height = 4, scale = 2.5)
ggsave("results/genera_highest_variance_heatmap_facets3.pdf",width = 6.5, height = 4, scale = 2.5)
ggsave("results/genera_highest_variance_heatmap_facets3.eps",width = 6.5, height = 4, scale = 2.5)
...
genus_abund %>% 
  ...
  geom_tile(color = "grey70") +
  #ggh4x::facet_nested(~plot_categories + gut_section,scales = "free_x") +
  ggh4x::facet_nested(~ gut_section + transplant_type + days_after_transplant ,scales = "free_x") +
  #scale_fill_viridis_c(na.value = "#ffffff") +
  ggh4x::scale_y_dendrogram(hclust = most_variant_genera_hclust) +
  scale_fill_viridis_c(trans = "log2",na.value = "#ffffff",labels = scales::label_number(accuracy = 0.01)) +
  theme_bw() +
...
ggsave("results/genera_highest_variance_heatmap_facet4.png",width = 6.5, height = 4, scale = 2.5)
ggsave("results/genera_highest_variance_heatmap_facets4.pdf",width = 6.5, height = 4, scale = 2.5)
ggsave("results/genera_highest_variance_heatmap_facets4.eps",width = 6.5, height = 4, scale = 2.5)

(genus_abund_plt_z <- genus_abund %>% 
  filter(Genus %in% most_variant_genera$Genus) %>% 
  group_by(Genus) %>% 
  mutate(Genus = factor(Genus, levels = hclust_order, ordered = TRUE),
         sample = factor(sample, levels = (sample_hclust_order), ordered = TRUE),
         z_score = scale(percent_abund)) %>% 
  ggplot(aes(sample, Genus,fill = z_score)) +
  geom_tile(color = "grey70") +
  #ggh4x::facet_nested(~plot_categories + gut_section,scales = "free_x") +
  #scale_fill_viridis_c(trans = "log10",na.value = "#ffffff",labels = scales::label_number()) +
  scale_fill_viridis_c(na.value = "#ffffff") +
  ggh4x::scale_y_dendrogram(hclust = most_variant_genera_hclust) +
  ggh4x::scale_x_dendrogram(hclust = sample_hclust) +
  theme_bw() +
    theme(axis.text.x = element_blank()) +
    labs(fill = "Genus abundance\nz-score",
         x = "Samples")
)
...
```{r}
(combined_plt_log_abund <- sample_section_bar / sample_day_bar / sample_type_bar / genus_abund_plt + plot_layout(guides = "collect",heights = c(0.1,0.1,0.1,3)) )

ggsave("results/genera_highest_variance_heatmap.png",width = 5, height = 4, scale = 2)
ggsave("results/genera_highest_variance_heatmap.pdf",width = 5, height = 4, scale = 2)
ggsave("results/genera_highest_variance_heatmap.eps",width = 5, height = 4, scale = 2)

(combined_plt_z_scores <- sample_section_bar / sample_day_bar / sample_type_bar / genus_abund_plt_z + plot_layout(guides = "collect",heights = c(0.1,0.1,0.1,2)) )

ggsave("results/genera_highest_variance_heatmap_z-scores.png",width = 5, height = 4, scale = 2)
ggsave("results/genera_highest_variance_heatmap_z-scores.pdf",width = 5, height = 4, scale = 2)
ggsave("results/genera_highest_variance_heatmap_z-scores.eps",width = 5, height = 4, scale = 2)
```
...
## Genus with biggest + day effect plots
```{r}
increasing_genera <- glm_res_w_tax_genus %>% 
  filter(`days_after_transplant.L:pval` < 0.05) %>% 
  top_n(25, `days_after_transplant.L:Est`) %>% 
  arrange(desc(`days_after_transplant.L:Est`)) %>% 
  mutate(rank = row_number())


rel_abund_long_genus <- rel_abund_long %>% 
  mutate(taxonomy_id = str_glue("d__{Domain};p__{Phylum};c__{Class};o__{Order};f__{Family};g__{Genus}")) %>% 
  filter(taxonomy_id %in% increasing_genera$taxonomy_id) %>% 
  left_join(metadata) %>% 
  left_join(increasing_genera %>% select(taxonomy_id, rank))


genus_id = "d__Bacteria;p__Firmicutes_A;c__Clostridia;o__Peptostreptococcales;f__Anaerovoracaceae;g__RGIG7444"

plot_increasing_genus <- function(genus_id){

  colors = c("#9a5ebd","#9dbfe0","#2083e6", "#dec8a6","#f09f26")

  current_phylum <- rel_abund_long_genus %>% 
    filter(taxonomy_id == genus_id) %>% 
    pull("Phylum") %>% 
    unique()
  
  current_genus <- rel_abund_long_genus %>% 
    filter(taxonomy_id == genus_id) %>% 
    pull("Genus") %>% 
    unique()
  
  current_rank <- rel_abund_long_genus %>% 
    filter(taxonomy_id == genus_id) %>% 
    pull("rank") %>% 
    unique()

  rel_abund_long_genus %>% 
    filter(taxonomy_id == genus_id) %>% 
    ggplot(aes(1, rel_abund, fill = plot_categories)) +
    ...
    labs(x = NULL,
         y = "% abundance (log10)", 
         fill = NULL,
         title = str_glue("Phylum: {current_phylum}\nGenus: {current_genus}"))
  
  fs::dir_create("results/day21_more_abund_genera")
    
  ggsave(str_glue("results/day21_more_abund_genera/{current_rank}--{current_genus}.png"), width = 4, height = 2, dpi = 600, scale = 1.5)
  ggsave(str_glue("results/day21_more_abund_genera/{current_rank}--{current_genus}.pdf"), width = 4, height = 2, dpi = 600, scale = 1.5)
  
}

walk(increasing_genera$taxonomy_id, plot_increasing_genus)
```
