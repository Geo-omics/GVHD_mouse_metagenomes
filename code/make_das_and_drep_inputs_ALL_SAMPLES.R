...
cm_res <- suppressMessages(read_tsv(paste0("data/omics/metagenomes/*/bins/all_raw_bins/checkm.txt"))) %>%
...
all_bins <- read_tsv(paste0("results/bins/*_bins.tsv"))

bins_to_consider <- cm_res %>% 
  mutate(qual = factor(qual, levels = c("Low", "Medium", "High"))) %>% 
  filter(!is.na(qual))

bin_contigs <- read_rds(paste0("data/omics/metagenomes/*/bins/contig_bins.rds"))
